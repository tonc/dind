...
echo "     • 可用镜像: $IMAGE_COUNT 个"
echo "              "
echo "  ⚠️  请记得定期清理不用的容器和镜像！"
echo "              "
echo "═══════════════════════════════════════════════════════════════"
echo ""
...
# 添加到 .bashrc
if ! grep -q "show-motd" /root/.bashrc; then
    echo "/usr/local/bin/show-motd" >> /root/.bashrc
fi
